            ...
            <div class="tab-content" id="projectDetailsTabContent">
                ...
                <div class="tab-pane fade p-3" id="project-files" role="tabpanel" aria-labelledby="files-tab">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span>
                                <i class="fas fa-file-alt me-2"></i> {{ $file->file_name }}
                            ...
                            <button type="button"
                                    class="btn btn-sm btn-outline-info me-2 view-file-modal-btn"
                                    data-bs-toggle="modal"
                                ...
                                <button type="button" class="btn btn-sm btn-outline-danger delete-file-btn" data-file-id="{{ $file->id }}" data-project-id="{{ $project->obfuscated_id }}">Delete</button>
                                @endif
                            </div>
                        </li>
                        @endforeach
                    ...
                    @endif
                    <small class="form-text text-muted mt-2">Files can be uploaded/managed from the project edit page.</small>
                </div> {{-- End tab-pane project-files --}}

                <div class="modal fade" id="fileViewerModal" tabindex="-1" aria-labelledby="fileViewerModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg"> <!-- Large modal for PDF preview -->
                        <div class="modal-content">
                        <div class="modal-header">
                            <h5 id="fileViewerModalLabel" class="modal-title">View File</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <!-- PDF will be displayed here -->
                            <iframe id="fileViewerFrame" src="" width="100%" height="500px" frameborder="0">
                            </iframe>
                        </div>
                        </div>
                    </div>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function(){
                        document.querySelectorAll('.view-file-modal-btn').forEach(function(el) {
                            el.addEventListener('click', function(){
                                var fileName = this.dataset.fileName;
                                var fileURL = this.dataset.fileUrl;

                                // Update modal title
                                document.getElementById('fileViewerModalLabel').innerHTML = "View File: " + fileName;

                                // Set src of iframe to PDF file
                                document.getElementById('fileViewerFrame').src = fileURL;
                            });
                        });
                    });
                </script>
                            ...
                            <div class="card h-100y position-relative border border-dark {{ $cardClasses }}"> {{-- h-100 to make cards in a row have equal height --}}
                                ...
                                <ul class="list-group list-group-flush">
                                    ...
                                    ($task->data_status == 0 ? 'bg-danger' : 'bg-secondary'))
                                            ...
                                            <p class="task-description text-muted small mt-2 mb-1">
                                                @if($project->data_status!=2 || $task->data_status!=2)
                                                    @if($task->assign_to_user_id==auth()->user()->id)
                                                        <a href=""><small class="text-primary">Edit</small></a> | <a href=""><small class="text-danger">Delete</small></a> | 
                                                    @endif
                                                @endif
                                                <button type="button"
            class="btn btn-sm btn-outline-secondary view-task-details-btn"
            data-bs-toggle="modal"
            data-bs-target="#taskDetailModal"
            data-task-id="{{ $task->obfuscated_id }}" {{-- Use obfuscated ID --}}
            data-project-id="{{ $project->obfuscated_id }}"> {{-- Pass project ID for context --}}
        Details
    </button></p>
                                        </div>
                                    </li>
                            ...
                            <div class="modal-body">
                                <div id="taskDetailContent">
                                    <div class="d-flex justify-content-center py-5">
                                        ...
                                        <span class="ms-2 text-muted">Loading task details...</span>
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                {{-- Optional: Edit Task Button (only for authorized users) --}}
                                <button type="button" class="btn btn-primary" id="editTaskButton" style="display:none;">Edit Task</button>
                            </div>
                        </div>
                    </div>
                </div>
                <script>
                    $(document).ready(function() {
                        ...
                        $('#taskDetailModal').on('show.bs.modal', function (event) {
                            ...
                            `);
                            $editTaskButton.hide(); // Hide edit button by default
                                ...
                                success: function(response) {
                                    if (response.success) {
                                        ...
                                        let htmlContent = `
                                            <div class="row mb-3">
                                                <div class="col-md-6">
                                                    <p><strong>Kanban Stage:</strong> ${task.project_stage.kanban_stage.name}</p>
                                                    <p><strong>Status:</strong> <span class="badge ${
                                                        task.status === 'completed' ? 'bg-success' :
                                                        (task.status === 'in_progress' ? 'bg-primary' :
                                                        (task.status === 'blocked' ? 'bg-danger' : 'bg-secondary'))
                                                    }">${task.status.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</span></p>
                                                    <p><strong>Due Date:</strong> ${task.end_date ? new Date(task.end_date).toLocaleDateString() : 'N/A'}</p>
                                                    <p><strong>Created By:</strong> ${task.created_by ? task.created_by.name : 'N/A'}</p>
                                                    <p><strong>Created At:</strong> ${task.created_at ? new Date(task.created_at).toLocaleString() : 'N/A'}</p>
                                                </div>
                                                <div class="col-md-6">
                                                    <p><strong>Assigned To:</strong>
                                                        ${task.assigned_to ? `
                                                            <img src="${task.assigned_to.avatar_url}" alt="${task.assigned_to.name}" class="rounded-circle me-1" width="24" height="24">
                                                            ${task.assigned_to.name}
                                                        ` : 'Unassigned'}
                                                    </p>
                                                    <p><strong>Description:</strong> ${task.description || 'No description provided.'}</p>
                                                    <p><strong>Update Log:</strong> ${task.update_log || 'No update log.'}</p>
                                                </div>
                                            </div>
                                        `;
                                            ...
                                            task.files.forEach(file => {
                                                htmlContent += `
                                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                                        ...
                                                        <div>
                                                            <a href="${file.file_url}" target="_blank" class="btn btn-sm btn-outline-info">View/Download</a>
                                                        </div>
                                                    </li>
                                                `;
                                            });
                                        ...
                                        $taskDetailContent.html(htmlContent);

                                        // Show Edit button if current user is project manager or assigned to task
                                        const currentUserId = {{ Auth::check() ? Auth::id() : 'null' }};
                                        if (currentUserId && (task.project.project_manager_id === currentUserId || task.assigned_to_user_id === currentUserId)) {
                                            $editTaskButton.show();
                                            // You'd set an action for this button, e.g., redirect to edit page or open edit modal
                                            // $editTaskButton.off('click').on('click', function() {
                                            //     window.location.href = `/your-project-edit-route/${task.obfuscated_id}/tasks/${taskId}/edit`;
                                            // });
                                        }
                                         ...
                                         <div class="col-md-12 mt-3">
                                            ...
                                            <div id="new-file-upload-container">
                                                    <div class="input-group mb-2 file-upload-item">
                                                        <input type="file" id="task_file" name="task_file" class="form-control" accept=".pdf,.doc,.docx">
                                                    </div>
                                            </div>
                                    </div>
                                </div>
